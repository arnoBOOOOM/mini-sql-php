    ...
    <body>
        ...
        <main>
            <div class="container .text-center .conatainer-fluid">
                <div class="text-center"><br><h1>Tweedehands!</h1><br><h4>Tweedehands auto's te koop</h4><br>
                  <?php
                  ...
                  $sql = "SELECT * from tweedehandswagens";
                  $result = $conn->query($sql);

                  if($result->num_rows > 0) {
                          //output data of each row
                          echo "<h3>tweedehands autos</h3><table><tr><th>id</th><th>merk</th><th>model</th><th>bouwjaar</th><th>brandstof</th><th>laatstekeuring</th><th>kleur</th><th>aantalKM</th></tr>";
                          while($row = $result->fetch_assoc()) {
                              echo "<tr><td>" . $row["idtweehandswagens"] . "</td><td>" . $row["merk"] . "</td><td>" . $row["model"] . "</td><td>" . $row["bouwjaar"] . "</td><td>" . $row["brandstof"] . "</td><td>" . $row["laatsteKeuring"] . "</td><td>" . $row["kleur"] . "</td><td>" . $row["aantalKilometer"] . "</td></tr>";
                          }
